...
export const WaitersManager: React.FC<WaitersManagerProps> = ({ business }) => {
  ...
  const pairingUrl = `${window.location.origin}/pair-waiter?biz=${business.slug}`;
      ...
      .on(
        ...
        (payload: any) => {
          ...
          if (payload.eventType === 'INSERT' && payload.new?.status === 'pending') {
            toast.info(`Yeni Garson Cihaz Talebi: "${payload.new.device_name}" onay bekliyor!`, {
              duration: 6000,
            });
          }
        }
  ...
  const handleApproveDevice = async (deviceId: string) => {
    ...
    if (!waiterId) {
      toast.error('Lütfen bu cihaza atanacak garsonu seçiniz veya önce yeni bir garson ekleyiniz.');
      return;
    }
          ...
          <div className="space-y-2 max-w-xl">
            ...
            <div className="flex flex-wrap items-center gap-2.5 pt-2">
              ...
              >
                ...
                <span>QR Kodu Ekranda Büyüt</span>
              </button>
                ...
                <div className="pt-1">
                  ...
                  {waiters.length === 0 ? (
                    <div className="text-xs text-rose-600 font-bold bg-rose-50 p-2 rounded-xl">
                      Önce aşağıdan "+ Yeni Garson Ekle" butonuna basarak bir garson tanımlamalısınız.
                    </div>
                  ) : (
                    <select
      ...
      <div className="bg-white border border-slate-200/80 rounded-3xl p-5 sm:p-6 shadow-xs space-y-4">
        ...
        {loading ? (
          ...
          <div className="py-8 text-center text-xs text-slate-400 font-medium">
            Henüz kayıtlı bir garson bulunmuyor. Yeni garson ekleyerek başlayabilirsiniz.
          </div>
        ) : (
      ...
      {isAddWaiterOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-sm p-6 shadow-2xl space-y-4 animate-in zoom-in-95">
            <div className="flex items-center justify-between">
              <h3 className="font-black text-sm text-slate-900">Yeni Garson Tanımla</h3>
