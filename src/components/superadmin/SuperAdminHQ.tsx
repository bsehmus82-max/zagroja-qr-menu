...
export const SuperAdminHQ: React.FC<{ onLogout: () => void }> = ({ onLogout }) => {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      // Realtime subscription for restaurants and support messages
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 selection:bg-indigo-500 selection:text-white">
        <form onSubmit={handleLogin} className="bg-slate-900 p-8 rounded-3xl w-full max-w-sm border border-slate-800 shadow-2xl animate-slide-up">
          <div className="flex justify-center mb-6">
            <div className="w-16 h-16 bg-gradient-to-tr from-indigo-600 to-blue-600 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-600/30">
          ...
          <button
            type="submit"
            className="w-full py-3.5 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white font-bold rounded-xl transition-all shadow-lg shadow-indigo-600/30 text-sm active:scale-95"
          >
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-slate-900 p-5 sm:p-6 rounded-3xl border border-slate-800 shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-600 to-blue-600 flex items-center justify-center text-white font-black shadow-lg shadow-indigo-600/30">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Tüm Kayıtlı İşletmeler & Canlı İletişim ({restaurants.length} İşletme)</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <button
              ...
              className="px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white rounded-xl font-bold text-xs flex items-center gap-2 shadow-lg shadow-indigo-600/30 transition-all active:scale-95"
            >
          ...
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="İşletme adı, kullanıcı adı veya kalıcı link ile ara..."
              ...
              <form onSubmit={handleCreateRestaurant} className="bg-slate-900 p-6 rounded-3xl border border-indigo-500/40 shadow-2xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 animate-slide-up">
                <div className="sm:col-span-2 lg:col-span-3 border-b border-slate-800 pb-3 flex items-center justify-between">
                  <h3 className="font-bold text-white flex items-center gap-2 text-sm">
                    ...
                    <span>Yeni İşletme / SaaS Hesabı Oluştur</span>
                  </h3>
                ...
                <div>
                  ...
                  <input required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Örn: Cafe Silvana" className="w-full bg-slate-800 border border-slate-700 text-white p-2.5 rounded-xl text-xs outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-400 mb-1 block">Kalıcı URL Slug *</label>
                  <input required value={form.slug} onChange={e => setForm({ ...form, slug: e.target.value })} placeholder="örn: cafe-silvana" className="w-full bg-slate-800 border border-slate-700 text-white p-2.5 rounded-xl text-xs outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-400 mb-1 block">Giriş Kullanıcı Adı *</label>
                  <input required value={form.username} onChange={e => setForm({ ...form, username: e.target.value })} placeholder="örn: silvana_admin" className="w-full bg-slate-800 border border-slate-700 text-white p-2.5 rounded-xl text-xs outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-400 mb-1 block">Giriş Şifresi *</label>
                  <input required value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder="örn: 123456" className="w-full bg-slate-800 border border-slate-700 text-white p-2.5 rounded-xl text-xs outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-400 mb-1 block">Masa Sınırı</label>
                  <input type="number" min={1} max={999} value={form.max_tables} onChange={e => setForm({ ...form, max_tables: parseInt(e.target.value) || 1 })} className="w-full bg-slate-800 border border-slate-700 text-white p-2.5 rounded-xl text-xs outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
                <div>
                ...
                {form.type === 'timed' && (
                  <div>
                    ...
                    <input type="number" min={1} value={form.days} onChange={e => setForm({ ...form, days: parseInt(e.target.value) || 30 })} className="w-full bg-slate-800 border border-slate-700 text-white p-2.5 rounded-xl text-xs outline-none" />
                  </div>
                )}
            ...
            <div className="space-y-3">
                  ...
                  {editingRestId === rest.id && (
                    <form onSubmit={(e) => handleEditRestaurant(e, rest)} className="mt-2 p-4 bg-slate-800/80 rounded-2xl border border-slate-700 grid grid-cols-1 sm:grid-cols-4 gap-3 animate-fade-in text-xs">
                      <div>
                        ...
                        <input type="number" min={1} max={999} value={editForm.max_tables} onChange={e => setEditForm({ ...editForm, max_tables: parseInt(e.target.value) || 1 })} className="w-full bg-slate-900 border border-slate-600 text-white p-2 rounded-xl" />
                      </div>
                      <div>
                      ...
                      {editForm.type === 'timed' ? (
                        <div>
                          ...
                          <input type="number" min={0} value={editForm.days} onChange={e => setEditForm({ ...editForm, days: parseInt(e.target.value) || 0 })} className="w-full bg-slate-900 border border-slate-600 text-white p-2 rounded-xl" placeholder="+30 gün" />
                        </div>
                      ) : <div />}
                      <div className="flex items-end gap-2">
                        <button type="submit" className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl">Kaydet</button>
              ...
              {filteredRestaurants.length === 0 && !isLoading && (
                <div className="bg-slate-900 rounded-3xl border border-slate-800 p-12 text-center text-slate-500">
                  ...
                  <p className="text-xs text-slate-500 mt-1">Yukarıdaki "Yeni İşletme Ekle" butonuna basarak ilk müşterinizi sisteme kaydedebilirsiniz.</p>
                </div>
              )}
            </div>
          </div>
