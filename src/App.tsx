...
export default function App() {
  ...
  const loadCustomerBusiness = async (slug: string) => {
    ...
    try {
      ...
      if (error || !data) {
        setCustomerError('Ýþletme veya QR menü bulunamadý.');
        return;
      }

      const biz = data as Business;
      if (biz.subscription_status === 'suspended') {
        setCustomerError('Bu iþletmenin menüsü geçici olarak kapalýdýr.');
        return;
      }

      setCustomerBusiness(biz);
    } catch {
      setCustomerError('Menü yüklenirken bir hata oluþtu.');
    } finally {
    ...
    if (customerError || !customerBusiness) {
      return (
        <div className="min-h-screen bg-neutral-950 flex flex-col items-center justify-center p-6 text-center">
          ...
          <h2 className="font-bold text-lg text-white mb-2">{customerError || 'Menü Bulunamadý'}</h2>
          <p className="text-xs text-neutral-400 max-w-sm mb-6">
            Lütfen masanýzdaki QR kodu tekrar okutunuz veya iþletme yetkilisine danýþýnýz.
          </p>
          <button
            onClick={() => {
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col justify-between selection:bg-brand-500 selection:text-white relative overflow-hidden">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full p-6 md:p-12 flex flex-col justify-center z-10 space-y-12">
        <div className="text-center space-y-4 max-w-2xl mx-auto">
          ...
          <p className="text-sm text-neutral-400 leading-relaxed">
            Sýfýr gecikme, tam veri izolasyonu, sanat eseri estetiðinde QR menüler, anlýk adisyon fiþi ve canlý garson çaðrý sistemi.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            <div>
              ...
              <p className="text-xs text-neutral-400 leading-relaxed">
                Platform sahibine özel þifreli alan. Ýþletme hesabý açma, masa limitleri, canlý destek ve toplu duyuru merkezi.
              </p>
            </div>
          ...
          >
            <div>
              ...
              <h3 className="font-bold text-base text-white mb-1">2. Ýþletme Admin Paneli</h3>
              <p className="text-xs text-neutral-400 leading-relaxed">
                Menü ve ürün düzenleme, "Tükendi" dondurma, yuvarlatýlmýþ QR üretici, kasa/POS, adisyon fiþi ve ciro Z-raporu.
              </p>
            </div>
          ...
          >
            <div>
              ...
              <h3 className="font-bold text-base text-white mb-1">3. Müþteri QR Menü</h3>
              <p className="text-xs text-neutral-400 leading-relaxed">
                Uygulamasýz tarayýcý arayüzü. Sadece kategoride görsel, içerik bazlý ürün listesi, Garson Çaðýr, Hesap Ýste ve Wi-Fi kopyala.
              </p>
            </div>
            <div className="mt-6 pt-4 border-t border-neutral-800 flex items-center justify-between text-xs font-bold text-emerald-400">
              <span>Menüyü Ýncele</span>
              <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition" />
            </div>
          </div>
        </div>
      ...
      <footer className="border-t border-neutral-900 p-6 text-center text-xs text-neutral-600 z-10">
        © 2026 Zagroja QR Menü Platformu. Tüm haklarý saklýdýr.
      </footer>
    </div>
  );
}
