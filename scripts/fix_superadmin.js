...
export const SuperAdminDashboard: React.FC<SuperAdminDashboardProps> = ({ onLogout }) => {
  ...
  const handleAddDays = async (biz: Business, additionalDays: number) => {
    ...
    if (!error) {
      ...
      alert(`${biz.name} işletmesine ${additionalDays} gün süre eklendi.`);
    }
  };
  ...
  const handleDeleteBusiness = async (biz: Business) => {
    if (!window.confirm(`"${biz.name}" işletmesini ve tüm menüsünü silmek istediğinize emin misiniz?`)) {
      return;
    }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-black text-lg tracking-tight text-white">ZAGROJA PLATFORM MERKEZİ</h1>
            ...
            <p className="text-xs text-neutral-400">Merkezi İşletme Yönetimi & Canlı İletişim Ağı</p>
          </div>
        </div>
      ...
      <main className="flex-1 p-6 max-w-7xl mx-auto w-full">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-neutral-900 border border-neutral-800 p-5 rounded-3xl">
                <span className="text-xs font-medium text-neutral-400">Toplam İşletme</span>
              ...
              <div className="bg-neutral-900 border border-neutral-800 p-5 rounded-3xl">
                <span className="text-xs font-medium text-neutral-400">Askıya Alınan</span>
            ...
            {loading ? (
              ...
              <div className="py-20 text-center bg-neutral-900/40 border border-dashed border-neutral-800 rounded-3xl p-8">
                ...
                <h3 className="font-bold text-white text-base">Henüz İşletme Bulunmuyor</h3>
                <p className="text-xs text-neutral-400 mt-1">
                  Sağ üstteki <strong>"Yeni İşletme Aç"</strong> butonuna tıklayarak ilk işletmeyi oluşturabilirsiniz.
                </p>
              </div>
            ) : (
                      ...
                      <div>
                        <div className="flex items-start justify-between gap-4 mb-4">
                          <div>
                            ...
                            <div className="text-xs text-neutral-400 mt-1 font-mono">
                              Kullanıcı: <span className="text-neutral-200 font-bold">{biz.username}</span> | Slug: {biz.slug}
                            </div>
                          </div>
                        ...
                        {isExpiringSoon && (
                          <div className="mb-4 p-2.5 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center gap-2 text-xs text-red-400">
                            ...
                            <span>Son 3 gün kaldı! Yenileme uyarısı gönderilebilir.</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="pt-4 border-t border-neutral-800 flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            ...
                            {biz.subscription_status === 'suspended' ? 'Askıdan Çıkar' : 'Askıya Al'}
                          </button>
        ...
        {activeTab === 'database' && (
          <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6 space-y-4">
            <h3 className="font-black text-lg text-white">Supabase Veritabanı Kurulumu</h3>
            <p className="text-xs text-neutral-400">
              Veritabanı tablolarının tam olarak çalışması için <code>supabase_schema.sql</code> dosyası projenizde hazırdır.
            </p>
          </div>
        )}
      </main>
